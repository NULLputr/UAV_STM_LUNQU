...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  if( phost->device.DevDesc.idProduct == Wireless1_PS2_PID && phost->device.DevDesc.idVendor==Wireless1_PS2_VID)
  {	  //无线PS2手柄的PID/VID
	  USBH_UsrLog("Wiredless1 PS2 device found!");
	  gamepad_brand = PS2_USB_Wiredless1;
	  GamePadInterface = &Wiredless_USB_PS2Gamepad;
	  HID_Handle->Init = USBH_HID_GamePadInit;
  }
  else if( phost->device.DevDesc.idProduct == Wireless2_PS2_PID && phost->device.DevDesc.idVendor==Wireless2_PS2_VID)
  {	  //无线PS2手柄的PID/VID
	  USBH_UsrLog("Wiredless2 PS2 device found!");
	  gamepad_brand = PS2_USB_Wiredless2;
	  GamePadInterface = &Wiredless_USB_PS2Gamepad;
	  HID_Handle->Init = USBH_HID_GamePadInit;
  }
  else
...
///////////////////////// 初始化、数据解码 ///////////////////////////////
//手柄通用输入报告
static uint8_t GamePadReportData[64] = { 0 }; //用于存放HID设备发送过来的数据
...
USBH_StatusTypeDef USBH_HID_PS2_Decode(USBH_HandleTypeDef *phost)
{
	...
	if (USBH_HID_FifoRead(&HID_Handle->fifo, (uint8_t* )GamePadReportData, HID_Handle->length) == HID_Handle->length)
	{
		...
		//用于调试
//		for(uint8_t i=0;i<HID_Handle->length;i++)
//		{
//			printf("%d  ",GamePadReportData[i]);
//		}
//		printf("\r\n");
		
		//无线PS2手柄数据解码
		if( gamepad_brand == PS2_USB_Wiredless1 || gamepad_brand == PS2_USB_Wiredless2  )
		{
			Wiredless_PS2_Decode(phost,GamePadReportData,HID_Handle->length);
			
			//GamePadInterface存放着原始数据解析后的值
//			printf("LX:%d  LY:%d\r\n",GamePadInterface->LX,GamePadInterface->LY);
//			printf("RX:%d  RY:%d\r\n\r\n",GamePadInterface->RX,GamePadInterface->RY);
		}
